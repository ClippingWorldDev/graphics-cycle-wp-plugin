(function ($) {
  ...
  var PANEL_NS = '.softroTeamStyleThreeTabs';
  var TABS_CONTROL_SELECTOR = '.elementor-control-softro_team_style_three_tabs, [data-setting="softro_team_style_three_tabs"]';
  var MEMBERS_CONTROL_SELECTOR = '.elementor-control-softro_team_style_three_members, [data-setting="softro_team_style_three_members"]';
  ...
  var MEMBER_TAB_SELECT_SELECTOR = 'select[data-setting="style_three_tab_label"]';
  ...
  function getTabsFromModel(model) {
    ...
    var tabsSetting = typeof settings.get === 'function'
      ? settings.get('softro_team_style_three_tabs')
      : null;

    if (!tabsSetting) {
      if (settings.attributes && settings.attributes.softro_team_style_three_tabs) {
        tabsSetting = settings.attributes.softro_team_style_three_tabs;
      } else {
        return [];
      }
    }
    ...
    if (tabsSetting.attributes && tabsSetting.attributes.softro_team_style_three_tabs) {
      return tabsSetting.attributes.softro_team_style_three_tabs;
    }
  ...
  function getTabsFromPanelRowTitles(panel) {
    ...
    panel.$el.find(TABS_ROW_TITLE_SELECTOR).each(function () {
      ...
      // Skip empty row labels and simple symbols from control buttons.
  ...
  function syncStyleThreeMemberTabOptions(panel, model) {
    ...
    if (!settings || 'style_three' !== settings.get('softro_team_genaral_style_selection')) {
      return;
    }
    ...
    var $selects = panel.$el.find(MEMBER_TAB_SELECT_SELECTOR);
    ...
    $selects.each(function () {
      ...
      // Refresh select2 UI only. Do not trigger a regular change event, which can
      // cause Elementor to continuously re-render the preview.
  ...
  function bindPanelDomSync(panel, model) {
    var syncDebounced = debounce(function () {
      syncStyleThreeMemberTabOptions(panel, model);
    }, 120);
    ...
    panel.$el.on(
      ...
      TABS_CONTROL_SELECTOR + ' .elementor-repeater-add, '
        ...
        + MEMBERS_CONTROL_SELECTOR + ' .elementor-repeater-add, '
        + MEMBERS_CONTROL_SELECTOR + ' .elementor-repeater-tool-remove, '
        + MEMBERS_CONTROL_SELECTOR + ' .elementor-repeater-tool-duplicate, '
        + MEMBERS_CONTROL_SELECTOR + ' .elementor-repeater-row-title',
      function () {
  ...
  $(window).on('elementor:init', function () {
    ...
    var onWidgetPanelOpen = function (panel, model) {
      if (!model || !model.get || model.get('widgetType') !== 'softro_team') {
        return;
      }
      ...
      if (settings && settings.on && settings.off) {
        settings.off('change:softro_team_style_three_tabs', null, model);
        settings.off('change:softro_team_genaral_style_selection', null, model);

        settings.on('change:softro_team_style_three_tabs', function () {
          syncStyleThreeMemberTabOptions(panel, model);
        }, model);

        settings.on('change:softro_team_genaral_style_selection', function () {
          syncStyleThreeMemberTabOptions(panel, model);
        }, model);
      }
    ...
    elementor.hooks.addAction('panel/open_editor/widget/softro_team', onWidgetPanelOpen);
  });
})(jQuery);
